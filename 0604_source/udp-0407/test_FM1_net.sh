...
#ip route table all
route add $5 dev FM1-mac9
route add $6 dev FM1-mac10
...
echo -ne "[FM1_MAC9 AND FM1_MAC10 $value Test-Module]: `date +'%Y-%m-%d %T'`\t0/$7"
...
then
	...
	if [ $? -ne 0 ]
	then
        echo -en  "\r[FM1-MAC9 $value Test-Module]: `date +'%Y-%m-%d %T'` `echo -ne "\t\033[0;31;1merror\033[0m"`/$7 \b\n" 
        exit 1;
	fi	
else
    echo -en  "\r[FM1-MAC9 AND FM1-MAC10 $value  Test-Module]: `date +'%Y-%m-%d %T'` `echo -ne "\t\033[0;31;1merror\033[0m"`/$7 \b\n" 
    exit 1;
fi
./udp-receive --threadnum $1 --startcpu $2 --length 1460 --targetip $6 --sourceip $4 --port 8080  --check 2>&1 | tee -a $tmp_detail10 >> $detail &
./udp-send    --threadnum $1 --startcpu $2 --length 1460 --targetip $5 --sourceip $3 --port 8080  --check 2>&1 | tee -a $tmp_detail1 >> $detail &

number=$1+$2
./udp-receive --threadnum $1 --startcpu $number --length 1460 --targetip $5 --sourceip $3 --port 8100  --check 2>&1 | tee -a $tmp_detail20 >> $detail &
./udp-send    --threadnum $1 --startcpu $number --length 1460 --targetip $6 --sourceip $4 --port 8100  --check 2>&1 | tee -a $tmp_detail2 >> $detail &
...
do
	...
	then
		    ...
		    error="[error]"
        	echo -en  "\r[FM1-MAC9 AND FM1-MAC10 $value  Test-Module]: `date +'%Y-%m-%d %T'` `echo -ne "\t\033[0;31;1m${failed}${error}\033[0m"`/$7 \b\n" 
	...
	then
		...
		then
            ...
            echo -en "\r[FM1-MAC9 AND FM-MAC10 $value Test-Module]: `date +'%Y-%m-%d %T'` `echo -ne "\t\033[0;32;1m$success\033[0m"`/$7 \b" 
...
killall udp-send > /dev/null 2>&1
killall udp-receive > /dev/null 2>&1
echo -ne "\r[FM1-MAC9 AND FM1-MAC10 $value Test-Module]: `date +'%Y-%m-%d %T'``echo -ne "\t\033[0;32;1mFinished\033[0m"`/$7 \b\n" 
