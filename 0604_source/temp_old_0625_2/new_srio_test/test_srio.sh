...
./srio_testc --passes 0  --data_type swrite --test_type $1   2>&1 | tee -a $tmp_detail  >> $detail &
...
rm -rf $tmp_detail
echo -en  "\r[SRIO $value Test-Module]: `date +'%Y-%m-%d %T'` `echo -ne "\t\033[0;31;1m $failed\033[0m"`/`echo -ne "\033[0;32;1mFinished${success}\033[0m"` \b" 
killall srio_testc > /dev/null 2>&1
