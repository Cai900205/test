...
void config_down(uint8_t option0,uint8_t option1 )
{
    char tmp[200];
    sprintf(tmp,"ifconfig %s down",config_info[option0][0]);
    system(tmp);
    sprintf(tmp,"ifconfig %s down",config_info[option1][0]);
    system(tmp);
}
...
int main(int argc,char **argv)
{
    ...
    int maxwokers=1;
    maxwokers = sysconf(_SC_NPROCESSORS_ONLN);
    for (i = 1; i < argc; i++)
	{
		char* arg = argv[i];
	...
	struct task_type task_send[100];
	struct task_type task_rece[100];

    if(workers>maxwokers||workers<0)
    {
    ...
    port= strtoull(config_info[inter2_op][4],NULL,10);
	for(i=0;i<workers;i++)
	{
		task_rece[i].adr.sin_family =AF_INET; 
		task_rece[i].adr.sin_port = htons(port);
		task_rece[i].adr.sin_addr.s_addr = inet_addr(config_info[inter1_op][3]); 
		task_rece[i].user.sin_family = AF_INET;
		task_rece[i].user.sin_port = htons(port);
		task_rece[i].user.sin_addr.s_addr = inet_addr(config_info[inter1_op][2]);
		task_rece[i].se_adr.sin_family = AF_INET;
		task_rece[i].se_adr.sin_port = htons(port);
		task_rece[i].se_adr.sin_addr.s_addr = inet_addr(config_info[inter2_op][3]);
		task_rece[i].se_user.sin_family = AF_INET;
		task_rece[i].se_user.sin_port = htons(port);
		task_rece[i].se_user.sin_addr.s_addr = inet_addr(config_info[inter2_op][2]);
		task_rece[i].time = time;
		task_rece[i].bind = bind;
        task_rece[i].check = check;
		task_rece[i].len= packet_length;
		task_rece[i].passes= passes;
		err = pthread_create(&task_rece[i].id,NULL,t_receive,&task_rece[i]);
		if (err) {
			printf("Port %d : Receive thread failed!\n",2-port);
                        fflush(stdout);
			exit(1);
	...
	for(i=0;i<workers;i++)
	{
		...
		pthread_join(task_rece[i].id,NULL);
	}
    config_down(inter1_op,inter2_op);
	return 0;
}
