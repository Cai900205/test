...
static void *t_srio_send(void *arg)
{
	...
	cpu_set_t cpuset;
        ...
        uint32_t i=0;
    int result=-1;
	struct timeval tm_start,tm_end,p_start,p_end;	
	CPU_ZERO(&cpuset);
        if(bind)
        {
...
/*end*/
                ...
                float diff1 = ((p_end.tv_sec -p_start.tv_sec)*1000000+(p_end.tv_usec-p_start.tv_usec))/1000000.0;
                if((diff1 > time)&&(passes))
                {
                    printf("SRIO_TEST SEND time over!\n");
                    fflush(stdout);
                    break;
                }            
         }
		...
		if(receive_num>use_num)
		{
			...
			for(k=use_num;k<(use_num+packet_num);k++)
			{
				...
				for(i=0;i<PACKET_LENGTH;i++)
				{
					...
					{
						error_count++;
				                printf("###Receive ERROR Data:%02x  Test Data:%02x\n",*p,data);
				                fflush(stdout);
					}
					p++;
				}
