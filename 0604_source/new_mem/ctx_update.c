...
void * memtest(void *arg)
{
  ...
  if((i=pthread_setaffinity_np(pthread_self(),sizeof(cpu_set_t),&cpuset))>0)
  {
	printf("cpu error!\n");
	exit(1);
  }
...
int main(int argc, char **argv) {
    ul loops, loop, i,thread_num;
    size_t pagesize, wantraw, wantmb, wantbytes, wantbytes_orig;
    char *memsuffix, *addrsuffix, *loopsuffix,*threadsuffix;
    ptrdiff_t pagesizemask;
    int exit_code = 0;
...
/* ctx add*/
	...
	thread_num = 1;
        ...
        errno=0;
	thread_num = strtoul(argv[optind],&threadsuffix,10);
        if (errno != 0) {
            fprintf(stderr, "failed to parse number of thread_number");
            usage(argv[0]); /* doesn't return */
        }
        if (*threadsuffix != '\0') {
            fprintf(stderr, "thread_number suffix %c\n", *threadsuffix);
            usage(argv[0]); /* doesn't return */
        }
	printf("thread_num:%d\n",thread_num);
    }
/*ctx end*/
    ...
    for(i=0;i<thread_num;i++)
    {
       ...
       if (err) {
		printf("t=memtest create thread failed!\n");
		exit(1);
       } 
   }
   for(i=0;i<thread_num;i++)
