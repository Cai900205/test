...
temp_interfce1=(fm1-mac5 fm1-mac9 fm1-mac10 fm2-mac5 fm2-mac9 fm2-mac10)
temp_ip=(192.168.10.1 192.168.20.1 192.168.20.2 192.168.10.2 192.168.30.1 192.168.30.2)
temp_test_ip=(192.168.10.11 192.168.20.11 192.168.20.22 192.168.10.22 192.168.30.11 192.168.30.22)
...
#echo -e "$@"
temp=`getopt  -l help,version,workers:,time:,passes:,interval:,interface1:,interface2:,bind  -- -n "$@"`
...
for ((i=0; i< ${#temp_interfce1[@]} ;i++));
do
    if [ $interface1 == ${temp_interfce1[i]} ]   
    then
        ip1=${temp_ip[i]}
        test_ip1=${temp_test_ip[i]}
    fi
done

for ((i=0; i< ${#temp_interfce1[@]} ;i++));
do
    if [ $interface2 == ${temp_interfce1[i]} ]   
    then
        ip2=${temp_ip[i]}
        test_ip2=${temp_test_ip[i]}
    fi
done
