...
static fvl_srio_ctable_t srio_ctable_context[4]={
{"srio0-chan0",0,0,0},{"srio0-chan1",0,1,0},
{"srio1-chan0",1,0,0},{"srio1-chan1",1,1,0}
};
...
void fvl_srio_rese_ctl(void *arg)
{
    ...
    while(1) 
    {
        ...
        else
        {
            ...
            {
                rese_num[priv->fd]=rese_num[priv->fd]+(count-ctl_count);
                ctl_count=count;
            }
        }
    }
...
int fvl_srio_channel_open(char *name)
{
    ...
    fvl_srio_ctrlblk_t *re_pscb,*re_cscb;
...
//end mode master
    ...
    re_cscb=&(temp_channel->rechanblk); //add rese
    ppool = &psrio->portpool[port_num];
...
// very important
    ...
    re_pscb=fvl_srio_getcb(port_num,bfnum+3);
    if(re_pscb==NULL)
    {
        FVL_LOG("port:%d channel:%d : dmadev init error.\n",port_num+1,bfnum);
        return -1;
    }
    re_cscb->dmadev=re_pscb->dmadev;
    re_cscb->bfnum=re_pscb->bfnum;
    re_cscb->port = re_pscb->port;
