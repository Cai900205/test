...
fvl_srio_head_info_t head_channel[FVL_CHAN_NUM_MAX];
...
void fvl_srio_rese_ctl(fvl_ctl_thread_t *priv)
{
    ...
    {
         rese_num[priv->fd]=rese_num[priv->fd]+(head_port[priv->port_num].data_se_cluster[(priv->fd%FVL_PORT_CHAN_NUM_MAX)].buf_num -ctl_count[priv->fd]);
...
// very important
    ...
    rvl=fvl_srio_recv_head(&head_p_arg);
    if(rvl!=0)
    {
...
//need vtop
    ...
    port_num = srio_ctable_context[fd].port;
    buf_num  = head_port[port_num].data_se_cluster[(fd%FVL_PORT_CHAN_NUM_MAX)].buf_num;
    buf_size = head_port[port_num].data_se_cluster[(fd%FVL_PORT_CHAN_NUM_MAX)].buf_size;
