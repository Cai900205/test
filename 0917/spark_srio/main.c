...
void* thread_channel_send(void *arg)
{
	...
	memset(port_data->dma_virt_base,i,Buf_size);
        ...
        if(diff>5)
        {
            ...
            printf("SEND: fd: %d length(byte): %-15u time(s): %-15f  avg MB/s: %-15f total_count:%lu \n",fd,Buf_size,diff,da_lu,total_count);
            fflush(stdout);
            total_count=0;
            gettimeofday(&tm_start,NULL);
        }
...
uint32_t test_data(uint8_t src, uint8_t *buf,uint64_t count ,uint8_t step)
{
    ...
    for(i=0;i<count;i++)
    {
       ...
       {
          error_count++;
          printf("###Receive ERROR Data:%d   Test Data:%u  option:%lu\n",*buf,src,i);
          fflush(stdout);
       }
       src=src+step;
       buf++;
    ...
    while(1)
    {
 	...
 	rvl=fvl_srio_read(fd,&rlen);
        if(rlen.len!=0)
        {
            ...
            {
                double da_lu=(total_count*Buf_size/1048576/diff);
                printf("RECV: fd: %d length(byte): %-15u time(s): %-15f  avg MB/s: %-15f total_count:%lu \n",fd,rlen.len,diff,da_lu,total_count);
                fflush(stdout);
                total_count=0;
                gettimeofday(&tm_start,NULL);
            } 
...
int main(int argc, char **argv)
{
   ...
   for(i=start;i<num;i++)
   {
       fd[i]=fvl_srio_channel_open(channame[i]);
       printf("###################fd:%d*****************\n",fd[i]);
       if(fd[i]<0)
       {
            return -1;
       }
       pthread_create(&fd_id[i],NULL,thread_channel_recv,&fd[i]);
  }
//   
  k=Chan_num;
  for(i=start;i<num;i++)
  {  
      fd[k]=fvl_srio_channel_open(channame[k]);
      printf("###################fd:%d*****************\n",fd[k]);
      if(fd[k]<0)
      {
           return -1;
      ...
      p_send[k].fd=fd[k];
      pthread_create(&fd_id[k],NULL,thread_channel_send,&p_send[k]);
      k++;
  }
