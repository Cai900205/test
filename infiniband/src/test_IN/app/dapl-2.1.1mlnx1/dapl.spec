...
%build
%configure CPPFLAGS=%{_CPPFLAGS} LDFLAGS=%{_LDFLAGS} %{_CONF} --enable-ext-type=ib %{?configure_options}
...
if [ $1 -gt 1 ]; then
	...
	service mpxyd start || true
elif [ $1 -gt 2 ]; then
	service mpxyd restart || true
fi

%preun
if [ -f /etc/init.d/mpxyd ]; then
	/sbin/chkconfig --del mpxyd &> /dev/null || true
	service mpxyd stop || true
fi
