...
my @remove_debs = qw( ofed-docs mlnx-ofed-kernel-utils mlnx-ofed-kernel-dkms infiniband-diags ibutils rds-tools srptools libsdp1 sdpnetstat perftest dapl-dev dapl1 dapl2-utils libdapl-dev libdapl2 dapl1-utils compat-dapl-dev compat-dapl1 libopensm-dev libopensm-devel libopensm opensm-doc opensm opensm-libs libopensm2-dev libopensm2 librdmacm1-dbg librdmacm-dev rdmacm-utils librdmacm1 libipathverbs1-dbg libipathverbs-dev libipathverbs1 ibacm-dev ibacm ibsim-utils ibsim libibmad
libibmad-dev libibmad1 libibmad-devel libibmad-static libibumad libibumad-dev libibumad-devel libibumad-static libibumad1 libibcm-dev libibcm1 libibdm-dev libibdm1 libumad2sim0 libmlx4 libmlx4-1 libmlx4-1-dbg libmlx4-dev libmlx5 libmlx5-1 libmlx5-1-dbg libmlx5-dev ibverbs-utils libibverbs1-dbg libibverbs-dev libibverbs1 ar-mgr cc-mgr dump-pr ofed-scripts mft kernel-mft-dkms mft-compat mft-devel mft-devmon mft-devmondb mft-int mft-tests mstflint ibdump mxm fca openmpi openshmem mpitests
knem knem-dkms ummunotify ummunotify-dkms libvma mlnx-en srptools iser-dkms srp-dkms libmthca-dev libmthca1 libmthca1-dbg);
...
sub mark_for_uninstall
{
	...
	if (not $selected_for_uninstall{$package}) {
		if (is_installed_deb $package) {
			...
			if ( not exists $remove_debs_hs{$_} and `ofed_info 2>/dev/null | grep -i $package 2>/dev/null` eq "" and $package !~ /ofed-scripts/) {
...
sub main
{
	...
	print_blue "Removing MLNX_OFED packages\n" unless($quiet);
	if ($dry_run) {
	...
	foreach (@remove_debs){
		foreach (get_all_matching_installed_debs($_)) {
			if (not $selected_for_uninstall{$_}) {
				...
				if ( not exists $remove_debs_hs{$_} and `ofed_info 2>/dev/null | grep -i $_ 2>/dev/null` eq "" and $_ !~ /ofed-scripts/) {
