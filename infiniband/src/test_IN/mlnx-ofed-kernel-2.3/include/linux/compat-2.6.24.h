...
/* In vmklinux kmem_cache_create takes 6 arguments, and is a macro */
#if defined(COMPAT_VMWARE)
#define kmem_cache kmem_cache_s
#undef kmem_cache_create
#define kmem_cache_create(name, size, offset, flags, ctor) \
	({							\
		if (ctor) __incompatible_kmem_cache_create();	\
		vmklnx_kmem_cache_create(&THIS_MODULE->primary_mem_info, \
					 name, size, offset, NULL, NULL); \
	})
#endif /* COMPAT_VMWARE */

/* 2.6.21 and 2.6.22 kmem_cache_create() takes 6 arguments */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,23) && !defined(COMPAT_VMWARE))
