...
int fvl_srio_init(fvl_srio_init_param_t *srio_param)
{
    ...
    uint32_t ctl_size=768;//FVLSRIO_CTL_SIZE;
...
//add debug    
            rvl=fsl_srio_set_targetid(sriodev,port_num,1,target1_id[port_num]);
            if(rvl!=0)
            {
                FVL_LOG("SRIO port %d set  target_id faile!\n",port_num);
                return -errno;
            }
            rvl=fsl_srio_set_targetid(sriodev,port_num,2,target1_id[port_num]);
            if(rvl!=0)
            {
                FVL_LOG("SRIO port %d set  target_id faile!\n",port_num);
                return -errno;
...
int fvl_srio_channel_open(char *name)
{
    ...
    FVL_LOG("#############################\n");
    FVL_LOG("port_num:%d uflag:%d\n",port_num,head_port[port_num].uflag);
    volatile uint32_t *flag= &head_port[port_num].uflag;
    while(1)
    {
        fflush(stdout);
        if(*flag)
...
/*
    re_arg.fd=fd;
    re_arg.port_num=port_num;
    re_arg.buf_virt=cpool->pwrite_ctl_result+HEAD_SIZE;
    rvl = pthread_create(&(temp_channel->chan_id), NULL,fvl_srio_rese_ctl, &re_arg);
    if (rvl) 
    {
        FVL_LOG("Create receive packet thread error!\n");
        return -errno;
...
//need vtop
    ...
    FVL_LOG("Slave available buf number:%d\n",(rese_num[fd]-send_num[fd]));
    ...
    fvl_srio_send(pscb->dmadev,src_phys,dest_phys,HEAD_SIZE);
    FVL_LOG("3#######\n");
    FVL_LOG("Slave send data:%d buf_num:%d\n",send_num[fd],buf_num);
    return 0;
}
