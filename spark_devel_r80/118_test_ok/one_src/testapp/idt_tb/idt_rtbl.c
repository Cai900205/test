...
static int rtbl_set_indir(int argc, char *argv[])
{
    PORT_ID dport;
    DEV_ID did;
    int ret=-1;
...
// indirect mode
// register 0x70/0x74
    idt_reg_w32(g_idtfd, IDTR_RTE_DESTID_CSR, did); 
    idt_reg_w32(g_idtfd, IDTR_RTE_PORT_CSR, dport); 
...
const cmd_decl_t cmd_rtbl_tbl[] =
{
    ...
    {"SetEntryIndir",rtbl_set_indir, "Set table entry indirectly | Param: <DID> <PORT_ID>"},
