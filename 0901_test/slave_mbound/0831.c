...
//mode slave	
    ...
    for(i=0;i<chan_num;i++)
    {
        rvl=fsl_srio_set_targetid(sriodev,port_num,i,target_id[port_num]);
        if(rvl!=0)
        {
...
void fvl_srio_recv_head(void *arg)
{
    ...
    while(1) 
    {
        if(!priv->op_mode)
        {
            ...
            fvl_srio_context_t  *psrio = &g_srio_context;
            uint32_t win_size=0;
            int win_law=0,chan_law=0,i=0,rvl=0;
            struct srio_dev *sriodev;
            uint32_t attr_read, attr_write;
            uint32_t chan_size=0,chan_num=0;
            attr_read = srio_test_win_attrv[3];
            attr_write = srio_test_win_attrv[0];
            ppool = &psrio->portpool[port_num];
            pcnt->re_flag=0;
            ...
            chan_size=pcnt->chan_size;
            chan_num=head_port[port_num].chan_num;
           ...
           // for(i=0;i<chan_num;i++)
            {
                i=0;
                fsl_srio_set_ibwin(sriodev, port_num, i+1, (ppool->write_result+chan_size*i),
                               (head_port[port_num].data_re_addr+chan_size*i), chan_law);
            }
...
//data
                for(i=0;i<chan_num;i++)
                {
                    fsl_srio_set_obwin(sriodev, port_num, i+1,
                                   ...
                                   head_port[port_num].data_se_addr+chan_size*i, chan_law);
