...
int fvl_srio_init(fvl_srio_init_param_t *srio_param)
{
    ...
    uint32_t win_size=srio_param->buf_size*srio_param->buf_num*srio_param->chan_number;
    uint32_t ctl_win_size=256*srio_param->buf_num*srio_param->chan_number;
    uint32_t target_id = srio_param->target_id;
    uint32_t win_law=0,ctl_law=0;
    struct srio_dev *sriodev;
//    fvl_dma_pool_t *port_dma_wp = srio_param->port_wr_buf;
    ...
    if((port_num >= FVL_SRIO_PORTNUM) || (port_num <0))
    {
    ...
    psrio->buf_num[port_num]=srio_param->buf_num;
    psrio->buf_size[port_num]=srio_param->buf_size;
    psrio->chan_size[port_num]=srio_param->chan_size;
    psrio->ctl_size[port_num]=srio_param->buf_num*256;
 ...
 //   ppool->write_data = port_dma_wp->dma_phys_base;
        
    ppool->pwrite_result = port_dma_wr->dma_virt_base;
//    ppool->pwrite_data = port_dma_wp->dma_virt_base;
...
//ctl
 ...
 //   memset(port_dma_wp->dma_virt_base,0,win_size);        
...
int fvl_srio_write(int fd, void *buf_virt)
{
//need vtop
    fvl_srio_ctrlblk_t *pscb;
    fvl_srio_portpool_t *cpool;
    ...
    fvl_srio_send(pscb->dmadev,src_phys,dest_phys,buf_size);
    send_num[fd]++;
}

// return read_num and addr

fvl_read_rvl_t fvl_srio_read(int fd)
{
    uint32_t num=0;
    uint32_t offset=0;
    fvl_read_rvl_t rvl;
