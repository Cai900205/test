...
static void *t_srio_receive(void *arg)
{
    ...
    head_dest = (uint64_t *)(ppool->port_info.range_start+FVL_SRIO_DMA_WINSIZE+bfnum*FVL_SRIO_CTL_BUFSIZE+tmid*send_size);//important
    pcnt  = (fvl_srio_ctl_info_t *)ppool->pwrite_ctl_result[bfnum];
    buf_virt  =(uint8_t *)ppool->pwrite_result[bfnum];//shou shuju receive data
    pclt = pcnt+tmid;
    ...
    void *copy_buf=NULL;
    int copy_ret=0;
 //   copy_ret=posix_memalign(&copy_buf,4096,FVL_SRIO_PACKET_LENGTH*256);
 //   if(copy_ret!=0)
   // {
     //  printf("posix error!\n");
      // return;
...
//          memcpy(copy_buf,buf_virt+offset,FVL_SRIO_PACKET_LENGTH*packet_num);
	  uint8_t *p=(uint8_t *)(buf_virt+offset);
          uint32_t error_count=0;
          for(dp=use_num;dp<(use_num+packet_num);dp++)
          {
              uint8_t pdata=*p;
              for(di=0;di<0x10000;di++)
              {
                   if(*p!=data)
                   {
                       error_count++;
                       printf("port:%d channel:%d Receive ERROR Data:%02x Test Data:%02x \n",port,bfnum,*p,data);
                       fflush(stdout);
                   }
                   p++;
              }
              if(error_count!=0)
              {
                   printf("port:%d channel:%d Receive ERROR Data:%02x Test Data:%2x error_number:%08x\n",port,bfnum,pdata,data,error_count);
                   fflush(stdout);
              }
              data++;
          }
          if(error_count==0)
          {
              printf("port:%d,channel:%d Data Right\n",port,bfnum);
              fflush(stdout);
          }
//	  gettimeofday(&tm_end,NULL);
