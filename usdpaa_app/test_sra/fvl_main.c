...
/*    sprintf(path1,"/mnt/test%d",index);
    ...
    int dequeue_num=-1;
    /* *buf=NULL;*/
    uint64_t test_data=0x0000002100000021+index*0x0100000001000000;
    uint32_t times=0;
    ...
    while(1)
    {
        ...
        if(dequeue_num != -1)
        {
            uint64_t *buf = (uint64_t *)(fqueue->buf+dequeue_num*FVL_SRIO_DMA_BLKBYTES);
		...
		{
			error_count++;
			printf("data:%lld   test_data:%lld\n",*buf,test_data);
		}
		buf++;
		if(test_data!=0xfffffffffffffff)
		...
		test_data=test_data+0x0300000003000000;
                count = 0;
            }
        }
    }
