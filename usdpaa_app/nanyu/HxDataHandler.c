...
// length 12360, equal to 1545 8byte
#define HX504_FRAME_COUNT       1545
//#define HX504_FRAME_COUNT       2055
#define HX504_FRAME_PAYLOAD     (HX504_FRAME_COUNT-1)
...
int hx504_frame_check(uint64_t data_header, uint64_t data_first, uint64_t data_last)
{
    ...
    uint64_t num_tail_expected = HX504_FRAME_PAYLOAD;
    uint64_t offset;
    int ret = 0;

    num_head_expected += frame_id * HX504_FRAME_PAYLOAD;
    num_tail_expected += frame_id * HX504_FRAME_PAYLOAD;
    offset = frame_id * HX504_FRAME_COUNT;

    /* Frame header not matched */
    data_header &=0xffffffff00000000ull;
	...
	printf("frame-%lu: last data error @ global offset %lu, data is %016lx, expect %016lx\n",
	       frame_id, offset + HX504_FRAME_PAYLOAD, data_last, num_tail_expected);
        ret = -3;
    }
#endif
...
void hx504_check(uint8_t* buf, int cnt, uint64_t delta)
{
    ...
    static uint64_t frame_rest = HX504_FRAME_COUNT;
    uint64_t *frame= 0;
    ...
    frame = (uint64_t *)buf;
    if(frame_rest != HX504_FRAME_COUNT) {
	data_last = frame[frame_rest - 1];

	if(frame_rest == HX504_FRAME_PAYLOAD)
	    last_first = frame[0];
...
/*	if(rvl < 0) {
    ...
    while(cnt >= HX504_FRAME_COUNT) {
	data_header = frame[0];
	data_first  = frame[1];
	data_last   = frame[HX504_FRAME_PAYLOAD];
...
/*	if(rvl < 0) {
	    ...
	    fvl_hex_dump("Before last data", frame + HX504_FRAME_COUNT - 16, 128);

	    if((cnt - HX504_FRAME_COUNT) < 16) {
		fvl_hex_dump("Buffer end", frame + cnt - 16, 128);
	    } else {
		fvl_hex_dump("After last data", frame + HX504_FRAME_COUNT, 128);
	    }
	}
*/
	cnt   -= HX504_FRAME_COUNT;
	frame += HX504_FRAME_COUNT;
    }
    ...
    frame_rest = HX504_FRAME_COUNT - cnt;
