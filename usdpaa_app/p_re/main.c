...
static void *t_srio_send(void *arg)
{
    ...
    buf_virt = (uint64_t *)ppool->pwrite_data[bfnum];//send data
    ...
    pclt = pcnt+tmid;
    uint64_t buf_number=0,usebuf_number=0,total_buf=8;
    uint32_t data=1;
    ...
    while (1) {
	    ...
	    for(;usebuf_number<buf_number;)
            {
	       uint64_t offset= (usebuf_number%total_buf)*(0x200000);
	       src_phys = buf_phys+offset;			
	       dest_phys = buf_dest+offset;
	       memset((buf_virt+offset),data,0x200000);
               fvl_srio_send(pscb, src_phys, dest_phys, 0x200000);
               data++;
...
static void *t_srio_receive(void *arg)
{
    ...
    memcpy(head_virt,&packet_info,send_size);
    fvl_srio_send(pscb, head_phys, head_dest, send_size);
    printf("send packet_info!\n");
    uint64_t receive_num=0,packet_num=0,use_num=0;
    uint64_t buf_number=8;
    uint64_t total_count=0;
    uint64_t count=0;
    struct timeval tm_start,tm_end;
    gettimeofday(&tm_start,NULL);
...
/*ctx add receive packet */
      uint64_t offset=(count%buf_number)*(0x200000);
