...
int fvl_srio_init(fvl_srio_context_t **ppsrio, uint32_t srio_type)
{
    ...
    for (i = 0; i < port_num; i++) {
		...
		else
		{
        	...
        	//ppool->read_result[j]  = phy_base + FVL_SRIO_DMA_WINSIZ;//add
        	//ppool->write_data[j]   = phy_base + FVL_SRIO_DMA_WINSIZE * 2+;
	    }
		...
		ppool->pwrite_ctl_result[j] = vir_base_ctl +FVL_SRIO_CTL_BUFSIZE*j;
        	...
        	//ppool->pread_result[j]  = vir_base + FVL_SRIO_DMA_WINSIZE+FVL_SRIO_BUFFER_SIZE*j;
        	//ppool->pwrite_data[j]   = vir_base + FVL_SRIO_DMA_WINSIZE * 2+FVL_SRIO_BUFFER_SIZE*j;
	    }

        FVL_LOG("Port %d: virt base=%p, phys base=%llx\n", i, vir_base, phy_base);
        if(i == 1)
	{
		fsl_srio_set_ibwin(sriodev, i, 1, ppool->write_result[0],FVL_SRIO_SYS_ADDR, LAWAR_SIZE_64M);
        	fsl_srio_set_ibwin(sriodev, i, 2, ppool->write_ctl_result[0],FVL_SRIO_CTL_ADDR, LAWAR_SIZE_4M);
	}
        else
        {
		fsl_srio_set_ibwin(sriodev, i, 2, ppool->write_ctl_result[0],FVL_SRIO_CTL_ADDR, LAWAR_SIZE_4M);
