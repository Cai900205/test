    ...
    while(1)
    {
       for(i=0;i<8;i++)
       {
   	  data_dest = ppool->port_info.range_start+bfnum*FVL_SRIO_DMA_BUFSIZE+i*FVL_SRIO_DMA_BLKBYTES;//important 
          ...
          diff = (tm_end.tv_sec-tm_start.tv_sec)+((tm_end.tv_usec-tm_start.tv_usec)/1000000.0);
          if(diff>5)
          {
             double da_lu=total_count*2/diff;
             printf("port:%d channel:%d length(byte): %-15u time(s):%-15f avg MB/s:%-15f total_count:%lld\n",port,bfnum,FVL_SRIO_DMA_BLKBYTES,diff,da_lu,total_count);
             total_count=0;
             gettimeofday(&tm_start,NULL);
          }
       }
