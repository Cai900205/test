...
void usage()
{
      ...
      printf("          --times         The every times of displaying rate\n");
...
static int cmd_translate(int32_t cmd_num, char **cmd_in,struct cmd_param_type *cmd_param)
{
	...
	for ( i = 1; i < cmd_num; i++)
	{
		...
		else if (!strcmp(arg, "--times") && i + 1 < cmd_num) 
		{
			cmd_param->test_num = strtoul(cmd_in[++i],NULL,10);
		}
...
static void cmd_format_print(void)
{
	...
	printf("srio_test --port --times --type --test_type --startcpu \n");
	printf("-----------------------------------------------------\n");
        fflush(stdout);
}
