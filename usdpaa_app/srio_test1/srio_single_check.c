...
#define SRIO_SYS_ADDR		0x20000000	/* used for srio system addr */
...
#define SRIO_POOL_PORT_OFFSET\
	(SRIO_WIN_SIZE * SRIO_POOL_PORT_SECT_NUM/2)
#define SRIO_POOL_SECT_SIZE	SRIO_WIN_SIZE /*POOLSIZE*/
#define SRIO_POOL_SIZE	0x4000000  /*dma pool size 64M*/
...
/*ctx add*/
	...
	for (i = port; i < (port+1); i++) {
		dma_addr_t port_phys_base =
			dmapool->dma_phys_base + SRIO_POOL_PORT_OFFSET * 0;
		...
		port_data[i].virt = (typeof(port_data[i].virt))
			(dmapool->dma_virt_base + 0 * SRIO_POOL_PORT_OFFSET);
...
/*ctx add*/
	...
	for(i=0;i<1;i++)
	{
		...
		err = fsl_dma_chan_init(&send_dmadev[i], 0, port);	
		if (err < 0) {
			error(0, -err, "%s(): fsl_dma_chan_init()", __func__);
			goto err_srio_connected;
		}
		err = fsl_dma_chan_init(&receive_dmadev[i], 1, port);
		if (err < 0) {
			error(0, -err, "%s(): fsl_dma_chan_init()", __func__);
			goto err_srio_connected;
		}
...
/*
	for(i=1;i<SEND_THREAD_NUM;i++)
	{
		...
		task_arg_receive[i].srio_type = cmd_param.test_srio_type;
		task_arg_receive[i].cpu = cmd_param.start_cpu;
