...
#define SRIO_ctl_ADDR		0x20000000	/* used for srio system addr */
...
/*ctx add*/
	...
	for (i = 0; i < port_num; i++) {
		...
		fsl_srio_set_ibwin(sriodev, i, 0,
	...
	/*	fsl_srio_set_ibwin(sriodev, i, 2,
		...
		if (fsl_srio_port_connected(sriodev) & (0x1 << i)) {
			fsl_srio_set_obwin(sriodev, i, 0,
				   port_data[i].port_info.range_start,
				   SRIO_SYS_ADDR, LAWAR_SIZE_16M);
			fsl_srio_set_obwin_attr(sriodev, i, 0,
        ...
        err = fsl_srio_set_targetid(sriodev,0,0,0x11);
	if(err!=0)
        {
		printf("sro set targetid  failed!\n");
                fflush(stdout);
	}
        err = fsl_srio_set_targetid(sriodev,1,0,0x14);
	if(err!=0)
        {
		printf("sro set targetid  failed!\n");
                fflush(stdout);
