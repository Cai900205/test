...
//	while (1) 
	...
	{	
		if(test_type)
		{
                   uint32_t offset= 0;
		   if(srio_type!=3){
                         src_virt = (void *)(send_data.virt.write_data_prep+offset);
                         dest_virt = (void *)(send_data.range_virt+offset);
		   }else{
                         src_virt = (void *)(send_data.virt.write_data_prep+offset);
                         dest_virt = (void *)(send_data.range_virt+offset);
		   }
...
/*ctx send*/
				...
				if(send_num == SEND_TOTAL_NUM)
				{
					...
					memcpy((send_data.virt.write_data_prep+SEND_NUM_OFFSET),&ctl_info,sizeof(struct srio_ctl)); 
                                        src_virt = (void *)(send_data.virt.write_data_prep+SEND_NUM_OFFSET);
                                        dest_virt = (void *)(send_data.range_virt+SEND_NUM_OFFSET);
					memcpy(dest_virt,src_virt,sizeof(struct srio_ctl));
					send_num=0;
                                        pi++;
				} 
			}			
...
static void *t_srio_receive(void *arg)
{
	...
	if(srio_type!=3){
                src_virt = (void *)(receive_data.virt.write_data_prep);
                dest_virt = (void *)(receive_data.range_virt);
	}else{
                src_virt = (void *)(receive_data.virt.write_data_prep);
                dest_virt = (void *)(receive_data.range_virt);
	}
...
//		receive_num = *(volatile uint32_t *)(receive_data.virt.write_recv_data+SEND_NUM_OFFSET);
	...
	//		memcpy(receive_data.virt.write_data_prep,&total_buf,sizeof(uint32_t));
			memcpy(src_virt,&ctl_info,sizeof(struct srio_ctl));
/*ctx send*/
                        memcpy(dest_virt,src_virt,sizeof(struct srio_ctl));
...
/*ctx add*/
	...
	for(i=0;i<2;i++)
	{
		...
		task_arg_send[i].port_data_thread.port_info.range_start = port_data[port].port_info.range_start+THREAD_WIN_SIZE*0; 
                task_arg_send[i].port_data_thread.range_virt = (void *)(port_data[port].range_virt+THREAD_WIN_SIZE*0); 
		/* cta end*/
		task_arg_send[i].port = i;
		task_arg_send[i].srio_type = cmd_param.test_srio_type;
		task_arg_send[i].cpu = cmd_param.start_cpu+i+1;/*bind cpu*/
	...
	for(i=0;i<2;i++)
	{
		...
		task_arg_receive[i].port_data_thread.range_virt = (void *)(port_data[port].range_virt+THREAD_WIN_SIZE*1); 
		/* cta end*/
		task_arg_receive[i].port = i;
		task_arg_receive[i].srio_type = cmd_param.test_srio_type;
		task_arg_receive[i].cpu = cmd_param.start_cpu+i+5;/*bind cpu*/
