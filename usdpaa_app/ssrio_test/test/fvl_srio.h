...
#define FVL_PORT_SRIO_NUM          8 //DMA MAX NUMBER
...
typedef struct fvl_srio_context {
    ...
    uint32_t buf_size[FVL_SRIO_PORTNUM];
    uint32_t buf_num[FVL_SRIO_PORTNUM];
    uint32_t chan_size[FVL_SRIO_PORTNUM];
    uint32_t ctl_size[FVL_SRIO_PORTNUM];
} fvl_srio_context_t;
...
int fvl_srio_write(int fd,void  *buf);
