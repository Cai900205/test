...
static void *t_srio_send(void *arg)
{
    ...
    pclt = pcnt+tmid;
    printf("cpu:#############%d\n",priv->cpu); 
    uint64_t buf_number=0,usebuf_number=0,total_buf=256;
    uint32_t data=1;
...
/*ctx add send packet */
    /*           printf("send buffer:%d\n",buf_number);*/
	       uint64_t offset= (usebuf_number%total_buf)*(0x10000);
...
/*ctx add receive packet */
      uint64_t offset=(count%buf_number)*(0x10000);
...
int main(int argc, char *argv[])
{
    ...
    for(i=0;i<1/*FVL_SRIO_BUFFER_NUMBER*/;i++)
    {
