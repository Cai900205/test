...
static void fvl_srio_recver(void *arg)
{
 ...
 /*   rvl = fvl_tcp_init(&tcp, tcp_ip, tcp_port);
    ...
    uint64_t test_data=0x0000002100000021;
	...
	total_count++;
        ...
        if(diff>5)
        {
            ...
            printf("port:%d channel:%d length(byte): %-15u time(s): %d  avg MB/s: %-15f total_count:%lld error_count:%ld\n",port,bfnum,FVL_SRIO_DMA_BLKBYTES,diff,da_lu,total_count,error_count);
            total_count=0;
            gettimeofday(&tm_start,NULL);	
        }  
    	tmp.FLA=pclt->FLA;
...
/*	printf("%02x\n",pclt->FLA);
        ...
        printf("bfnum=%d\n",bfnum);*/
        uint64_t *q=(uint64_t *)(buf_virt+offset);
        for(test_times=0;test_times < 0x40000;test_times++)
	{
		if(*q != test_data)
		{
			error_count++;
			printf("data:%16x test_data:%16x\n",*q,test_data);
		}
		q++;
		test_data=test_data+0x0000000100000001;				
	}
	if(error_count != 0)
	{
		printf("packet error:%ld\n",error_count);
	} 
