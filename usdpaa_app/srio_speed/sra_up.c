...
#define PACKET_LENGTH 0x200000	/*packet length*/
#define BUFFER_NUM 4		/*buffer number*/
#define SEND_TOTAL_NUM 2	/*SEND_NUMber*/
/*ctx end*/
...
static void *t_srio_send(void *arg)
{
	...
	int atb_flag=0;
	uint32_t test_packets_num = args->test_num;
	cpu_set_t cpuset;
        double speed=0.0;	
	CPU_ZERO(&cpuset);
	...
	while (1) {
/*ctx add send packet */
			uint32_t offset= (count%total_buf)*0x200000;
			if(srio_type!=3){
			...
			fsl_dma_direct_start(dmadev, src_phys, dest_phys,0x200000);
			...
			if (!test_packets_num && cmd_param.test_num) {
				...
				printf("CPU:%d PORT:%d %s %-15u %s %-15f %s %-15f %s %-15f min: %-15f\n",args->cpu,port,
				...
				(atb_to_seconds(atb_clock_min(atb_clock),
					atb_multiplier) * 1000000000.0),
				size * 8.0 /
				(atb_to_seconds(atb_clock_max(atb_clock),
					atb_multiplier) * 1000000000.0));
				fflush(stdout);
				
				atb_clock_finish(atb_clock);
				test_packets_num = args->test_num;
				atb_flag = 0;
			}
	}
...
/*ctx add*/
	...
	for(i=0;i<1;i++)
	{
		...
		task_arg_send[i].test_num = cmd_param.test_num;
