...
int fvl_srio_init(fvl_srio_init_param_t *srio_param)
{
    ...
    uint32_t ctl_size=FVL_CTL_SIZE;
    ...
    uint32_t ctl_win_size=ctl_size*chan_num;
...
void fvl_srio_recv_head(void *arg)
{
    ...
    while(1) 
    {
		...
		else if(priv->op_mode == 1)
		{
            ...
            head_channel[priv->num].u_flag=1;
		    break;
		}
    }
    return;
...
void fvl_srio_recv_ctl(void *arg)
{
    ...
    while(1) 
    {
		...
		else
		{
			...
			{
                receive_num[priv->fd]=receive_num[priv->fd]+(head_port[priv->port_num]-ctl_count);
				ctl_count=0;
			}
...
//
    ...
    rvl = pthread_create(&(temp_channel->chan_id), NULL,fvl_srio_recv_ctl, &re_arg);
    if (rvl) 
    {
        FVL_LOG("Create receive packet thread error!\n");
        return -errno;
...
//need vtop
    ...
    buf_size = head_port[port_num].buf_num;
...
int fvl_srio_read_feedback(int fd,int num)
{
	...
	rfreeback_num[fd]=rfeeback_num[fd]+num;
