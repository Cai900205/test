...
int pcie_user_read64(int fd, int offset, unsigned long long* value)
{
	int rv = -1;
	iomsg_t iomsg;
	memset(&iomsg, 0, sizeof(iomsg_t));
	iomsg.bar = GENERIC_PCIE_BAR_INDEX;
	iomsg.offset = offset;
...
int pcie_user_write64(int fd, int offset, unsigned long long value)
{
	int rv = -1;
	iomsg_t iomsg;
	memset(&iomsg, 0, sizeof(iomsg_t));
...
int pcie_user_read32(int fd, int offset, unsigned long* value)
{
	int rv = -1;
	iomsg_t iomsg;
	memset(&iomsg, 0, sizeof(iomsg_t));
...
int pcie_user_write32(int fd, int offset, unsigned long value)
{
	int rv = -1;
	iomsg_t iomsg;
	memset(&iomsg, 0, sizeof(iomsg_t));
